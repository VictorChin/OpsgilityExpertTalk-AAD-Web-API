...
function callMarseiAPI(endpoint, accessToken, callback) {
    const headers = new Headers();
    const bearer = `Bearer ${accessToken}`;
    ...
    console.log('request made to Marsei API at: ' + new Date().toString());
...
async function readBankAccount() {
    if (myMSALObj.getAccount()) {
        const response = await getTokenPopup(bankTokenRequest).catch(error => {
            console.log(error);
        });
        callMarseiAPI(marseiApiConfig.BankBalanceEndpoint, response.accessToken, updateUI);
